...
# TODO: doesn't work for 'eq' command
current_equipment = [r"You see " + __player +
                     r" (?:the [A-Za-z'\-]+)\.\n?\r?(?:(?:.+?\.\n?\r?)+)?((?:.+?:.+\n?\r?)+)"]
...
# This occurs on success as well
...
__Numbered_mob = r"(?P<numbered_mob>[A-Z](?:he " + \
    __numbers_opt + ")?[a-z '-]+) "
...
# __the_mob2 = "(P<mob>[\w '-]+) "
__mob = r"[a-z '-]+"
__the_mob2 = r"(P<mob>(?:[A-Z][a-z '-]+)|(?:the " + \
    __numbers_opt + __mob + ")) "
...
# __Three_possible_mob_strings = r"((?P<mob1>The ([A-Z][a-z '-]+ )+)|(?:The " + __numbers_opt + r"((?P<mob2>[a-z '-]+) ))|(?P<mob3>[A-Z][A-Za-z '-]+ ))"
__Three_possible_mob_strings = r"((?P<mob1>The( [A-Z][a-z'-]+)+)|(?:The " + \
    __numbers_opt + r"(?P<mob2>[a-z '-]+))|(?P<mob3>[A-Z][A-Za-z '-]+))"
# __three_possible_mob_strings = r"((?P<mob1>The ([A-Z][a-z '-]+ )+)|(?:the " + __numbers_opt + r"(?P<mob2>([a-z '-]+ ))|(?P<mob3>[A-Z][A-Za-z '-]+ )))"
# __three_possible_mob_strings = r"((?P<mob1>The ([A-Z][a-z '-]+ )+)|(?:the " + __numbers_opt + r"((?P<mob2>[a-z '-]+) ))|(?P<mob3>[A-Z][A-Za-z '-]+ ))"
...
mob_aggro = [__Three_possible_mob_strings +
             " attacks you\."]  # TODO: alert here(?)
# mob_wandered = ["(?:The " + __numbers_opt + ")?(?P<mob>.+?) just wandered to the (?P<exit>.+?)\."
mob_wandered = [__Three_possible_mob_strings +
                r" just wandered to the (?P<exit>[a-z ]+)\."]
...
# no 1st/2nd numbers in arrived, but Two/Three is possible
mob_arrived = [r"(?P<mobs>[A-Z][a-z]* [a-z '-]+?) just arrived\."]
...
mob_joined1 = [__three_possible_mob_strings +
               " joins in the fight!"]  # A mob standing there joins
mob_joined2 = [__three_possible_mob_strings +
               " decides to join in on the fight!"]  # A mob wanders in and joins
mob_attacked = [  # TODO: do any mobs wield weapons? (different text)
    ...
    __Three_possible_mob_strings + \
    r" grabs you and gouges you for (?P<d>\d+) damage\.",
    __Three_possible_mob_strings + \
    r" tries to gouge you, but you shake (him|her|it) off\.",
    __Three_possible_mob_strings + \
    r" painfully head-butts you for (?P<d>\d+) damage\.",
    __Three_possible_mob_strings + \
    r" tries to grab you, but you break free of (his|her|its) grasp\.",
    __Three_possible_mob_strings + \
    r" lashes out and thumps you for (?P<d>\d+) damage\.",
    ...
    # TODO - missing no dmg version of this one
    __Three_possible_mob_strings + \
    r" charges at you and butts for (?P<d>\d+) damage\.",
    __Three_possible_mob_strings + \
    r" casts a [A-Za-z\-]+ spell on you for (?P<d>\d+) damage\."
]
...
mob_died = [r"Your attack overwhelms " +
            __three_possible_mob_strings + " and (?:s?he|it) collapses!"]
# it_fled = ["The (" + numbers + " )?(?P<mob_name>.+?) flees to the (.+?)\."]
# mob_fled = ["(?:The ?(" + __numbers + " )?)?(?P<mob_name>.+?) flees to the (?P<exit>.+?)\."]
mob_fled = [__Three_possible_mob_strings +
            r" flees to the (?P<exit>[a-z ]+)\."]
mob_defeated = [r"Your enemy, " +
                __three_possible_mob_strings + r" has been defeated\."]
you_died = [r"You are overwhelmed by " +
            __three_possible_mob_strings + r"'s attack and you collapse!"]
loot_blocked = [__Three_possible_mob_strings +
                r" won't let you take anything\."]
...
mob_aura = [__Three_possible_mob_strings +
            r" glows with a " + __aura + r" aura\."]
...
# The spiv won't let you take anything.
loot_blocked = [__Three_possible_mob_strings +
                r"won't let you take anything\."]
teleported = [r"### " + __player +
              r"'s body is teleported away to be healed\."]
...
# Neato dark blue colored text
red_mist_fades = [r"The red mist fades from your sight\."]
...
attack_hit = [
    "(?s)You swing with your .+?, hacking (?:the )?(" +
    __numbers + " )?(.+?)\s+for\s+(?P<d>\d+)\s+damage\.",
    "(?s)You slice (?:the )?(" + __numbers +
    " )?(.+?) for (?P<d>\d+)\s+damage\s+with\s+your\s+.+?\.",
    "You slash at (?:the )?(" + __numbers +
    " )?(.+?) and hit for (?P<d>\d+) damage\.",

    "You chop at (?:the )?(" + __numbers + " )?(.+?) for (?P<d>\d+) damage\.",
    "(?s)You stab (?:the )?(" + __numbers +
    " )?(.+?) with your .+?,\s+causing\s+(?P<d>\d+)\s+damage",
    "You lunge at (?:the )?(" + __numbers +
    " )?(.+?), striking for (?P<d>\d+) damage\.",

    "You lash out and thump (?:the )?(" + __numbers +
    " )?(.+?) for (?P<d>\d+) damage\.",
    "You punch (?:the )?(" + __numbers + " )?(.+?) for (?P<d>\d+) damage\.",
    "You kick (?:the )?(" + __numbers + " )?(.+?) for (?P<d>\d+) damage\.",
    "You head-butt (?:the )?(" + __numbers +
    " )?(.+?) for (?P<d>\d+) damage\.",
    "You grab (?:the )?(" + __numbers +
    " )?(.+?) and gouge (him|her|it) for (?P<d>\d+)\s+damage\.",

    "(?s)You smash your .+? into (?:the )?(" + __numbers +
    " )?(.+?),\s+causing\s+(?P<d>\d+)\s+damage\.",
    "You heave your .+? at (?:the )?(" + __numbers +
    " )?(.+?),\s+smashing\s+(him|her|it)\s+for\s+(?P<d>\d+)\s+damage\.",
    "You bludgeon (?:the )?(" + __numbers + " )?(.+?) for (?P<d>\d+) damage\.",

    "You lunge at (?:the )?(" + __numbers +
    " )?(.+?), hitting them for (?P<d>\d+) damage\.",
    "You swing your .+?, striking for (?P<d>\d+) damage\.",
    "(?s)You sweep (?:the )?(" + __numbers +
    " )?(.+?) with your .+?\s+for\s+(?P<d>\d+)\s+damage\.",

    "Your missile slams into (?:the )?(" + __numbers +
    " )?(.+?) for (?P<d>\d+) damage\.",
    "(?s)You attack (?:the )?(" + __numbers +
    " )?(.+?) with your .+?,\s+striking\s+for\s+(?P<d>\d+)\s+damage\.",
    "You use your .+? to strike (?:the )?(" + __numbers +
    " )?(.+?)\s+for\s+(?P<d>\d+)\s+damage\.",
    "Your touch completely overwhelms the (.+?)!"
]
attack_miss = [
    "You hack with your .+?, but your blow swings wide of the mark\.",
    "You slice your .+? at (?:the )?(" + __numbers +
    " )?(.+?),\s+but\s+miss\.",
    "You slash at (?:the )?(" + __numbers + " )?(.+?), but miss\.",

    "You chop at (?:the )?(" + __numbers + " )?(.+?) but fail to hit them\.",
    "(?s)You try to stab (?:the )?(" + __numbers +
    " )?(.+?) with your .+?,\s+but\s+miss\.",
    "You lunge wildly at (?:the )?(" + __numbers +
    " )?(.+?) but mistime\s+the\s+strike\.",

    "You lash out at (?:the )?(" + __numbers + " )?(.+?), but miss\.",
    "You swing a wild punch at (?:the )?(" +
    __numbers + " )?(.+?), but it misses\.",
    "You kick at (?:the )?(" + __numbers + " )?(.+?), but fail to hurt them\.",
    "You grab at (?:the )?(" + __numbers +
    " )?(.+?), but (s?he|it) escapes your grasp\.",
    "You try to gouge (?:the )?(" + __numbers +
    " )?(.+?), but can't get a good grip\.",

    "(?s)You swing your .+? at (?:the )?(" +
    __numbers + " )?(.+?),\s+but\s+miss\.",
    ...
    "(?s)You sweep at (?:the )?(" + __numbers +
    " )?(.+?) with your .+?,\s+but\s+miss\.",

    "Your missile arcs towards (?:the )?(" + __numbers +
    " )?(.+?), but fails\s+to\s+hit\s+them\.",
    "(?s)You attack (?:the )?(" + __numbers +
    " )?(.+?) with your .+?,\s+but\s+miss\.",
...
# Gets a positive of the off-hand message
you_wield = [r"You wield (an?|some) (?P<weapon>[A-Za-z ']+)\."]
...
weapon_break = [
    r"Your (?P<weapon>[A-Za-z' ]+?) breaks and you have to remove it\."]
...
heavier_second = [
    r"Your second weapon cannot be heavier than your primary one\."]
...
# TODO: this is made up and needs to be checked
not_for_sale = [r"That item is not for sale\."]
cant_carry = [
...
repair = [
    r"The smithy hands an? (?P<weapon>[A-Za-z' ]+?) back to you, almost good as new\."]
# TODO: The smithy takes \d+ gold pieces from you.
darnitall = [
    r'"Darnitall!" shouts the smithy, "I broke another\. Sorry la(d|ss)\.']
...
get_ring = [r"(?s)You get " + __items +
            r"?an? [a-z]+ ring(([a-zA-Z0-1-',\s]+\.)|(\.))"]
